...
export const GET = auth(async (req: NextAuthRequest) => {
  if (
    ...
    return profileImageResponseError({
...
export const POST = auth(async (req: NextAuthRequest) => {
  ...
  if (files.length === 0) {
    return profileImageResponseError({
      message: 'No files was uploaded to the server.',
    })
  }

  if (!files[0].type.startsWith('image/')) {
    return profileImageResponseError({
      message: `Invalid file type "${files[0].type}". Please upload an image.`,
    })
  }
  ...
  const { image, error } = await uploadProfileImage(
    files[0],
    req.auth.speaker._id,
  )
  if (error) {
